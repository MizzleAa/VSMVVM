...
{
    public class ConfusionMatrixTooltipBehavior : Behavior<ConfusionMatrix>
    {
        public static readonly DependencyProperty TooltipElementProperty =
            DependencyProperty.Register(nameof(TooltipElement), typeof(FrameworkElement), typeof(ConfusionMatrixTooltipBehavior),
                new PropertyMetadata(null, OnTooltipElementChanged));

        public static readonly DependencyProperty TitleElementProperty =
            DependencyProperty.Register(nameof(TitleElement), typeof(TextBlock), typeof(ConfusionMatrixTooltipBehavior),
                new PropertyMetadata(null));

        public static readonly DependencyProperty BodyElementProperty =
            DependencyProperty.Register(nameof(BodyElement), typeof(TextBlock), typeof(ConfusionMatrixTooltipBehavior),
                new PropertyMetadata(null));

        public static readonly DependencyProperty TooltipMarginProperty =
            DependencyProperty.Register(nameof(TooltipMargin), typeof(double), typeof(ConfusionMatrixTooltipBehavior),
        ...
        {
            if (d is ConfusionMatrixTooltipBehavior self) self.EnsureTooltipReady();
        }
        ...
        private void OnHover(object sender, ConfusionMatrixHoverState e)
        {
            if (TooltipElement == null) return;
            if (!e.HasValue) { Hide(); return; }
            if (TitleElement != null) TitleElement.Text = $"{e.RowLabel} → {e.ColumnLabel}";
            if (BodyElement != null) BodyElement.Text = $"Value: {e.Value:0.##}";
